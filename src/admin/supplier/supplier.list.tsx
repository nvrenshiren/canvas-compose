...
import DeliveryForm from './supplier.list.delivery.form'
import ListForm from './supplier.list.form'
...
export default class extends React.Component {
    ...
    },
    {
      ...
      formatter: function(item: any) {
        if (item.orderItems.length > 0 && item.orderItems[0].pic) {
          let src =
            '/api/transfer/thumbs/' + item.orderItems[0].pic + '?w=30&h=30'
          return (
      ...
      formatter: (item: any) => {
        return (
          <ButtonGroup size="small" key={item.order.oid + 'oper'}>
            ...
            {Common.getOption(!!0, item.order).delivery && (
              <Button
                onClick={() => {
                  let service = SupplierService.deliveryOrder
                  ModalForm.open({
                    key: item.oid + 'edit',
                    title: '发货',
                    type: 'edit',
                  ...
                  <Col className="oper-item">
                    <RangePicker
                      onChange={(moment, dataArr) => {
                        this.state.form.startTime = new Date(
                          dataArr[0]
                        ).getTime()
  ...
  getData(
    ...
    let info: any = {}
    let modules = []
    if (this.state.admin.info) {
      info = this.state.admin.info
      modules = this.state.admin.info.modules
    }
    let service = OrderService.updateDelivery
    if (_.indexOf(window.userInfo.modules, 8) > -1 && info.issuper === false) {
      SupplierService.listOrder(
        Object.assign(
