...
import OrderWayForm from './order.way.form'
//antd组件
        ...
        <ButtonGroup size="small" key={item.pid + 'oper'}>
          ...
          <Button>
            <Popconfirm
              title={'确认删除?'}
              onConfirm={() => {
                OrderService.deleteDelivery({ did: item.did }).then((res: any) => {
                  if (res.stat === 'OK') {
                    EventPlus.emit('listRefresh')
                  }
                })
...
export default class extends React.Component {
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ModalForm.open({
                              ...
                              module: 'product',
                              formCom: OrderWayForm,
                              service: OrderService.insertDelivery,
                              callback: () => {
                                EventPlus.emit('listRefresh')
                              }
                            })
  ...
  getData(pageIndex: number = 1, pageSize: number = 20, pagination: boolean = true) {
    ...
    OrderService.listDelivery({ pageIndex: pageIndex, pageSize: pageSize }).then((res: any) => {
      if (res.stat === 'OK') {
        store.dispatch({
          type: 'LIST_ADD',
          list: {
