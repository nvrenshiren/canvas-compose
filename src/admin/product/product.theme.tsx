...
import ProductThemeForm from './product.theme.form'
        ...
        <ButtonGroup size="small" key={item.pid + 'oper'}>
          ...
          <Button>
            <Popconfirm
              title={'确认删除该商品?'}
              onConfirm={() => {
                ProductService.deleteTheme([item.tid]).then((res: any) => {
                  if (res.stat === 'OK') {
                    EventPlus.emit('listRefresh')
                  }
                })
...
export default class extends React.Component {
  ...
  getData(
    ...
    ProductService.listTheme({ pageIndex: pageIndex, pageSize: pageSize }).then(
      (res: any) => {
        if (res.stat === 'OK') {
          store.dispatch({
            type: 'LIST_ADD',
