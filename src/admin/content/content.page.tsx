        ...
        <ButtonGroup size="small" key={item.pid + 'oper'}>
          <Button
            onClick={() => {
              ModalForm.open({
                ...
                service: ContentService.updateCategory,
                callback: () => {
          ...
          <Button>
            <Popconfirm
              title={'确认删除?'}
              onConfirm={() => {
                ContentService.deleteCategory({ spcids: [item.spcid] }).then((res: any) => {
                  if (res.stat === 'OK') {
                    EventPlus.emit('listRefresh')
                  }
                })
...
export default class extends React.Component {
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ModalForm.open({
                              ...
                              module: 'product',
                              formCom: contentPageForm,
                              service: ContentService.addCategory,
                              callback: () => {
                                EventPlus.emit('listRefresh')
                              }
                            })
  ...
  getData(pageIndex: number = 1, pageSize: number = 20, pagination: boolean = true) {
    ...
    ContentService.listCategory({ pageIndex: pageIndex, pageSize: pageSize }).then((res: any) => {
      if (res.stat === 'OK') {
        store.dispatch({
          type: 'LIST_ADD',
          list: {
