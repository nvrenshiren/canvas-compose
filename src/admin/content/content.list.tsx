...
import contentPageForm from './content.list.form'
// antd组件
...
const Columns = [
  ...
  },
  {
    ...
    formatter: (item: any) => {
      return (
        <a href={'#/page/' + item.spid} target="_blank">
          /page/
          {item.spid}
        </a>
      )
        ...
        <ButtonGroup size="small" key={item.pid + 'oper'}>
          <Button
            onClick={() => {
              ModalForm.open({
                ...
                item: item,
                module: 'order',
                formCom: contentPageForm,
                service: ContentService.updateSinglePages,
                callback: () => {
          ...
          <Button>
            <Popconfirm
              title={'确认删除?'}
              onConfirm={() => {
                ContentService.deleteSinglePages({ spids: [item.spid] }).then(
                  (res: any) => {
                    if (res.stat === 'OK') {
                      EventPlus.emit('listRefresh')
                    }
...
export default class extends React.Component {
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ModalForm.open({
                              ...
                              module: 'product',
                              formCom: contentPageForm,
                              service: ContentService.addSinglePages,
                              callback: () => {
                                EventPlus.emit('listRefresh')
                              }
                            })
  ...
  getData(
    ...
    ContentService.listSinglePages({
      pageIndex: pageIndex,
      pageSize: pageSize
    }).then((res: any) => {
      if (res.stat === 'OK') {
