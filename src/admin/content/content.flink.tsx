...
import contentFlinkForm from './content.flink.form'
import contentFlinkItemForm from './content.flink.item.form'
        ...
        <ButtonGroup size="small" key={item.pid + 'oper'}>
          ...
          <Button>
            <Popconfirm
              title={'确认删除?'}
              onConfirm={() => {
                AdService.deleteAd([item.adid]).then((res: any) => {
                  if (res.stat === 'OK') {
                    EventPlus.emit('listRefresh')
                  }
                })
...
export default class extends React.Component {
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ModalForm.open({
                              ...
                              type: 'add',
                              module: 'content',
                              formCom: contentFlinkItemForm,
                              service: AdService.addAdTag,
                              callback: () => {
                                EventPlus.emit('listRefresh')
                              }
                            })
  ...
  getData(pageIndex: number = 1, pageSize: number = 20, pagination: boolean = true) {
    ...
    AdService.listAllAd({ pageIndex: pageIndex, pageSize: pageSize }).then((res: any) => {
      if (res.stat === 'OK') {
        store.dispatch({
          type: 'LIST_ADD',
          list: {
